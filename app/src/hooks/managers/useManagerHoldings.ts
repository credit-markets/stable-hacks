...
/**
 * Hook to get manager holdings with pagination
 * @param managerAddress - Manager address
 * @param page - Page number (default: 1)
 * @param pageSize - Page size (default: 5)
 */
export function useManagerHoldings(
  ...
  return useQuery<HoldingsResponse, Error, HoldingsResponse>({
    queryKey: ["manager-holdings", managerAddress, page, pageSize],
    queryFn: async () => {
      if (!managerAddress) {
        return {
          data: [],
      ...
      const response = await api.get<HoldingsResponse>(
        `/managers/${managerAddress}/holdings`,
        {
